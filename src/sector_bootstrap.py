...
def _reduced_basis(oplist, d, tol=1e-10, budget_bytes=6e9, verbose=False):
    ...
    if verbose:
        print(f"    reduction: n_h={n_h} touched Hermitian ops, d^2={d*d}, nnz(F)={F.nnz}, route={route}, "
              f"est. peak memory {est/1e9:.2f} GB")
    if est > budget_bytes:
...
def solve_sector(model, k, L_adj=2, L_sing=2, L_eom=3, add_Q=True, gs=True, ward=False,
    ...
    """dry_run=True builds the operators and returns the memory estimate without forming the Gram matrix."""
    t0 = time.time()
    ...
    # ---- reduce to the span of touched operators
    touched = [I, Hk] + [X for blk in blocks for row in blk['X'] for X in row] + eom
    if dry_run:
        Fd = _hermitian_parts(touched, d)
    ...
    r = W.shape[1]
    def coords(Xs):
        """rows c(X) with phi(X) = c(X) . y  for a list of operators X."""
        Xf = sp.vstack([_flat(X, d) for X in Xs]).tocsr()          # n_X x d^2
        T = (F.conj() @ Xf.T).toarray()                             # <O_a, X>  (n_h x n_X)
        return (W.T @ T).T                                          # n_X x r  (complex)
    y = cp.Variable(r)
    cons = [cp.real(coords([I])[0] @ y) == 1]
    for blk in blocks:
        m = len(blk['X'])
        Cm = coords([blk['X'][a][b] for a in range(m) for b in range(m)])
        M = cp.reshape(Cm @ y, (m, m), order='C')
        cons.append((M + cp.conj(M.T)) / 2 >> 0)
    if eom:
        Ce = coords(eom)
        cons += [cp.real(Ce) @ y == 0, cp.imag(Ce) @ y == 0]
    obj = cp.Minimize(cp.real(coords([Hk])[0] @ y))
    prob = cp.Problem(obj, cons)
    try:
        prob.solve(solver=solver)
    except Exception:
        prob.solve(solver='SCS', eps=1e-9, max_iters=200000)
    exact = float(np.linalg.eigvalsh(Hk.toarray())[0]) if d <= 3000 else float('nan')
    return dict(k=k, d=d, r=r, E_bound=prob.value, E_exact=exact, status=prob.status, nEOM=len(eom),
                time=time.time() - t0)
