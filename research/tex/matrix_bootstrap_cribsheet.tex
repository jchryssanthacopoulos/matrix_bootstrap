...
\maketitle
\vspace{-0.9cm}
\noindent\textit{Scope.} Everything here is drawn from the project library: Han--Hartnoll--Kruthoff \cite{HHK} (the founding paper), Lin--Zheng \cite{LZ24,LZ25}, Lin's TASI lectures \cite{TASI}, Laliberte--McPeak \cite{LM}, Cho--Gabai--Sandor--Yin \cite{CGSY}, Klebanov et al.\ \cite{KMPT}, Chen \cite{Chen}, Chang et al.\ \cite{CCSY}, Turiaci--Witten \cite{TW}. Equation numbers in brackets refer to those papers. Section~\ref{sec:ferm} contains project-specific statements (marked $\star$) that are derived in \texttt{docs/derivations.md} or verified numerically in this repository, not taken from the literature.
...
HHK (7), (15); LM (1.1)--(1.4); LZ25 (5); TASI (95)--(96) \\
Stationarity (EOM) &
...
HHK (2), (9); LM (1.5); LZ25 (4); TASI (100) \\
...
HHK (5) \& fn.\ [20]; LM (2.14) \& fn.\ 2; LZ24 (57)--(58), Fig.\ 4; TASI \S6.2 \\
...
HHK (14), (S37)--(S38); LZ24 (11)--(12), (29); TASI (101) \\
...
HHK (11)--(12); LZ24 (13); LZ25 after (1); LM (3.12)--(3.13), \S4 \\
...
CGSY (1.5)--(1.6); LZ25 (6); TASI (102)--(104); LM (3.20) \\
KMS / thermal state &
For $\rho\propto e^{-\beta H}$: $\beta C-A^{1/2}\log(A^{1/2}B^{-1}A^{1/2})A^{1/2}\succeq0$ with $A=\ev{O_i^\dagger O_j}$, $B=\ev{O_jO_i^\dagger}$, $C=\ev{O_i^\dagger[H,O_j]}$ (equivalent to KMS given stationarity); made SDP-representable by a rational relaxation of the matrix logarithm. Trivialises for gauged planar models with traced operators only. &
CGSY (1.6), (2.4)--(2.11), \S6 \\
...
\item Why it converges: if $\phi(\mathbf 1)=1$, $\phi(O^\dagger)=\phi(O)^*$ and $\phi(O^\dagger O)\ge0$ for \emph{all} $O$, then $\phi=\Tr\rho\,(\cdot)$ for a genuine state; adding $\phi(OH)=E\phi(O)$ makes it an eigenstate (HHK fn.\ [20]). For a finite fermionic Hilbert space ``all $O$'' is a finite set, so the bootstrap is exact at the top level.
...
\item \textbf{SUSY floor and how to lift it} ($\star$). With $Q,\Qb$ in the basis, $\phi(H)\ge0$ is free. In a lifted sector the SDP must additionally learn that annihilators act first on the sector ground state, e.g.\ $\phi(Q\Qb)=0$ at $N_\Psi=0$. In a basis of elementary letters this follows from $\phi(N_\Psi)=0$, $\phi(\Psi_{ij}\Psib_{ji})\ge0$ and Cauchy--Schwarz. In a basis of \emph{gauge-invariant single traces only} it does not: at $N=3$, levels 3 and 4, all lifted sectors of the single-matrix model return the trivial bound $0$ (verified 2026-09-15). Open-index (adjoint-valued) operators are required in the positivity matrix; the precedent is the ungauged-MQM bootstrap of CGSY \S3.1, whose adjoint Gram matrix is decomposed into $U(N)$ tensor structures (3.5) with no gauge constraint imposed.
...
\item Single-matrix fermionic model (Chen; this project): $E\in\{0,18\}$ ($N{=}2$), $\{0,18,45,72\}$ ($N{=}3$); $C_2^{\max}=N(N^2-1)/3$; $\dim r_*=3^{N(N-1)/2}$; BPS window $[N(N-1)/2,N(N+1)/2]$. Three-matrix model at $N=2$: 972 BPS states at $N_\Psi\in\{5,6,7\}$, counts $243{:}486{:}243$, 168 distinct energies (ED, reproduced 2026-09-15). Compare the $\hat q=3$ super-Schwarzian/SYK prediction of three charges in ratio $\cos(\pi k/3)=1{:}2{:}1$ (TW \cite{TW} (3.10); FGMS \cite{FGMS} (5.7)) and the TW chaos test: $r$-ratio of singular values of $Q_k$ per charge pair, $\beta=2$.
\end{itemize}

\begin{thebibliography}{99}\small
\bibitem{HHK} X.~Han, S.~A.~Hartnoll, J.~Kruthoff, \emph{Bootstrapping Matrix Quantum Mechanics}, Phys.\ Rev.\ Lett.\ 125 (2020) 041601, arXiv:2004.10212.
...
\bibitem{CGSY} M.~Cho, B.~Gabai, J.~Sandor, X.~Yin, \emph{Thermal Bootstrap of Matrix Quantum Mechanics}, arXiv:2410.04262.
...
\bibitem{TW} G.~J.~Turiaci, E.~Witten, \emph{$\mathcal N=2$ JT Supergravity and Matrix Models}, arXiv:2305.19438.
\bibitem{FGMS} W.~Fu, D.~Gaiotto, J.~Maldacena, S.~Sachdev, \emph{Supersymmetric SYK models}, Phys.\ Rev.\ D 95 (2017) 026009, arXiv:1610.08917.
\end{thebibliography}
\end{document}
