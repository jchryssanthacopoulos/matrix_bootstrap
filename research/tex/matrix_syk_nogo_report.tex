...
The conclusion does not depend on the couplings, the gauge group, or the supercharge degree. (i) \emph{Couplings:} the cohomology depends on $C$ only through $\omega=\mathrm{Alt}(C)\in\Lambda^q\mathbb C^p$, and only through its $GL(p)$ orbit. At $p=3$, $\dim\Lambda^3\mathbb C^3=1$, so $\omega$ is unique up to scale: Chen's tensor is the \emph{only} non-degenerate three-flavour cubic supercharge, and randomising $C$ either reproduces it or sets $\omega=0$ and widens the window to $3N+1$ (sampling $30$ random cyclic tensors: $26$ and $4$ respectively). More generally random couplings are \emph{optimal} --- ranks are lower semicontinuous in $\omega$, so specialising can only enlarge the cohomology and widen the support --- so every width quoted here is the best case. Disorder, the ingredient that makes SYK work, has nowhere to act. (ii) \emph{Casimir:} descending the spectrum at $N=3$ through complete Casimir tiers from $C_2=48$ to $C_2=24$ --- thirteen irreps, up to Fock multiplicity $110\,080$, a factor $215$ above the maximal irrep --- the support is width $4$ in every single case. (iii) \emph{Gauge group:} $su(N)$ removes one Cartan direction and narrows the window by exactly one, so $su(3)$ (rank $2$) concentrates with $h=9,18,9$ while $su(4)$ (rank $3$) does not; the controlling parameter is $\rank G$. (iv) \emph{Degree:} raising $q$ does not help, for the reason given next.
...
\item \textbf{Beyond the maximal weight.} The factorisation~\eqref{eq:factorise} is derived at the maximal weight for $u(N)$; it fails for $su(N)$, where Cartan directions straddle slots --- visible in the anomalous $su(4)$ profile $13,81,81,13$. Empirically the window matches the maximal one throughout the $N=3$ spectrum down to $C_2=24$, but this is not derived.
\item \textbf{The low-Casimir bulk.} The macroscopic-index complexes lie at low Casimir --- at $N=4$ the largest multiplicity is $6.6\times10^9$ at $C_2=20$ --- where weight spaces reach $10^9$ and a genuinely sparse rank routine is needed.
\item \textbf{Chaos without concentration.} The three-flavour $H$ is \emph{not} a Casimir function, so the model may still be chaotic and have a super-Schwarzian near-BPS spectrum even without a concentrated BPS sector. Nothing here bears on that; Altland--Zirnbauer statistics of the singular values of $Q_k$ are now cheap to extract weight by weight.
