...
### 8.4 Assessment of reach (2026-09-18) and hand-over

* **Hilbert-space route: $N=2$ only, and essentially complete.** Remaining: $k=4$ (almost-BPS, $E_0=0.088$) at level 4–5, which needs the direct solver interface; $k=5,6,7$ are BPS and exact via the SUSY floor; $k\ge8$ by particle–hole symmetry. The reducer cannot be taken to $N=3$ (block dimensions to $2\times10^7$; the isotypic dimension $D$ would be astronomically large) — the "$U(3)$ highest weights" and "never form $H_k$ densely" extensions are retired.
* **Empirical rule so far: a lifted sector becomes tight one level after it leaves the SUSY floor, and the level at which it leaves grows with the particle number** ($k=2$: 3; $k=3$: 3→4; $k=4$: $>3$; single matrix $N=4$ edge $k=5$: $>5$). Working hypothesis: level $\approx k+1$. With six letters the largest cone is $\sim840$ at level 5 and $\sim4500$ at level 6, so level 5–6 is the ceiling of any word-based bootstrap, capping the reachable sectors at $k\lesssim4$–$5$ *at any $N$*.
* **Consequence.** The sector that decides R-charge concentration — the first lifted one adjacent to the window, many particles and a small gap — is exactly where the bootstrap is weakest. The next engine (`docs/trace_bootstrap_plan.md`) therefore targets the fixed-$k$, any-$N$ regime with an exact-finite-$N$ trace formulation; the window at $N=3$ goes to sparse Lanczos.
