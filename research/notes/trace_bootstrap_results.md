...
**Assembly.** A fixed-$N$ parallel build (`TraceSDP.fixed_N`, 8 forked workers, coefficients evaluated at $N$, no $N$-polynomials stored) produces the level-$(4,3,4)$ problem for $N=3$, $k=3$ in ~25 s: 27 adjoint cones (largest $168,168,168,111,\dots$) + singlet cones, **51 012 monomials** once EOM/sector rows are restricted to Gram entries of total length $\le6$ (`L_eom_gram=6`; the length-8 entries' commutators would add $\sim10^6$ length-10 monomials that the Hilbert-space experience says carry almost nothing). Two further reductions were needed and are now standard: reality imposed at the variable level (Hermitian monomials lose their imaginary part, $m^\dagger=c\,m'$ pairs share one unknown; $-$45 % unknowns, $-$100 k rows) and an absolute floor on evaluated coefficients (a numerically-zero row must never be unit-normalised — the D4.3 trap again, found via `check_exact` on $k=1$). Final size: $n=55\,115$ real unknowns at level 3 ($70.3533$ reproduced, 3.7 GB), and at level 4 $n\approx5\times10^4$, $\sim3\times10^4$ equality rows, $\sim4\times10^5$ cone rows.

**Solvers.** (i) Clarabel: killed by the watchdog at 13 GB during KKT setup. Interior-point codes carry a dense $n_{\rm svec}^2$ block per PSD cone; a 168-dimensional complex cone is a 336-dimensional real one with $n_{\rm svec}=56\,616$, i.e. $\sim25$ GB per cone. Level 3 (cones $\le28\to56$) was fine; level 4 is out of reach for this class of solver at any memory we have. (ii) SCS with its direct (QDLDL) linear solver: >30 min in the KKT factorisation of the $\sim4.5\times10^5$-dimensional system without a single iteration, 8 GB. (iii) SCS with the CG (indirect) solver: converges hopelessly slowly even on toy instances. (iv) SCS with the Accelerate LDL backend: being tested. **Also fixed along the way:** SCS needs `rho_x=10^{-3}` (default $10^{-6}$) on these rank-deficient equality blocks, otherwise it stalls (N=2, k=1: 20 000 iterations vs 5 925 to "solved").
