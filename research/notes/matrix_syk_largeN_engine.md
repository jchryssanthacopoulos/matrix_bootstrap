...
**Addendum (2026-09-18) — route 1 completed at $N=2$, and what replaces "the engine".** Route 1 of §3 (covariant, fixed-$N_\Psi$ moment matrix) was built as the Hilbert-space sector bootstrap (derivations D3–D4) and is exact for the three-matrix model at $N=2$ in every sector tested (`sector_bootstrap_results.md` §8). It cannot go beyond $N=2$ ($d_k$ up to $2\times10^7$ at $N=3$). The large-$N$ continuation is now specified in `docs/trace_bootstrap_plan.md`: the same constraint set as identities among trace-word expectation values, **exact at finite $N$** with $N$ a numerical coefficient (no factorisation — the planar limit is blind to concentration, which is $1/N$-subleading, cf. §3 above). Realistic reach: fixed $k\lesssim4$–$5$ at any $N$; the half-filling window at $N=3$ is a sparse-Lanczos problem (plan §8). The single-trace $H$ of §2 (with the corrected constant, D1) and its three-matrix generalisation D2.13 are the inputs to that engine, as anticipated.
