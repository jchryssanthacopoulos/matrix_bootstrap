...
- [ ] **BPS-exclusion bootstrap**: add rows $\phi(XQ)=\phi(\bar QX)=0$ (all words $X$ up to level) to both engines; infeasibility ⇒ no BPS states in sector $k$. Test at $N=2$, $k=4,8$ (outside the window) at levels 2–3; then sweep $k$ at $N=3$ (index predicts: all $k\equiv0$ empty, window $\{13,14\}$) and $N=4$ (window $\{23,24,25\}$ if TW-like). Where exclusion leaves one degree per complex, BPS counts = the refined index (`results/data/refined_index_N*_p3.json`).
