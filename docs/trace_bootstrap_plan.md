...
- **M1 — algebra + harness** (`trace_algebra.py`, tests): canonical forms, (P.1), (ii), commutators, sector rows, finite-$N$ relations; all verified against explicit operators at $N=2,3$. *Acceptance:* zero identity failures on $10^3$ random words up to length 6.
...
## 8. Parallel track (separate plan): sparse Lanczos at $N=3$

The window question at $N=3$ ($n=27$, half filling $13.5$; single-matrix-like window would be $N+1=4$ sectors, SYK-like 3) is a sector-resolved sparse-Lanczos computation: $d_k\le2\times10^7$, matrix-free application of $H$ from the Jordan–Wigner representation, with the free diagonal reductions ($\mathbb Z_3$, $U(3)$ Cartan weights, $N_\Psi$). It also carries the Turiaci–Witten singular-value chaos test (Q2) and the per-irrep BPS counts (Q1b–d). This is the highest-value physics computation available and does not depend on the trace engine; it should be scheduled alongside M1–M3. Memory must be estimated first (vectors of $2\times10^7$ complex numbers are $0.3$ GB each; Lanczos needs a handful).
